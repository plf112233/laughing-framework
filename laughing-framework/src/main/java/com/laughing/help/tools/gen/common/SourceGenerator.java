...
public class SourceGenerator {
    ...
    private String genJavaVOTemplateName = "gen-java-vo.vm";

    private String genJavaQueryTemplateName = "gen-java-query.vm";
    ...
    public GenMetaInfo genSqlMap(Class<?> clazz, Writer out, GenDaoInfo genDaoInfo) {
        ...
        if (GenDaoInfo.DATA_FRAMEWORK_NAME_MYBATIS.equals(genDaoInfo.getFrameWorkName())) {
            genFromTemplate(context, out, sqlMapMybatisTemplateName);
        } else {
    ...
    public GenMetaInfo genDAO(Class<?> clazz, Writer out, GenDaoInfo genDaoInfo) {
        ...
        context.put("paramName", genMetaInfo.getDoAlias());
        if (GenDaoInfo.DATA_FRAMEWORK_NAME_MYBATIS.equals(genDaoInfo.getFrameWorkName())) {
            genFromTemplate(context, out, genMybatisDaoTemplateName);
        } else {
    ...
    public String genJavaAOImpl(Class<?> domainClass, String serviceContext, Writer out) {
        ...
        domainUpper = StringUtil.uppercaseFirstLetter(StringUtil.getLastBefore(domainUpper, DomainType.DTO));
        ...
        context.put("daoFullClassName", StringUtil.getLastBefore(genMetaInfo.getDaoFullClassName(), DomainType.DTO));
        ...
        context.put("voSimpleParam", StringUtil.lowercaseFirstLetter(genMetaInfo.getVoSimpleClassName()));
        context.put("querySimpleParam", StringUtil.lowercaseFirstLetter(genMetaInfo.getQuerySimpleClassName()));
    ...
    public String genJavaVO(Class<?> domainClass, String actionContext, Writer out) {
        Map<String, Object> context = MapUtil.newHashMap();
        String domainUpper = StringUtil.uppercaseFirstLetter(DaoGenUtil.getDoAlias(domainClass));
        domainUpper = StringUtil.uppercaseFirstLetter(StringUtil.getLastBefore(domainUpper, DomainType.DTO));
        String aoClassName = domainUpper + "VO";

        GenMetaInfo genMetaInfo = new GenMetaInfo(domainClass, idName);
        String baseActionPackage = genMetaInfo.getBaseBizPackage() + ".entity";
        String targetActionPackage;
        String actionContextPackage;
        ...
        templateSourceGenator.genSource(context, out, genJavaVOTemplateName);
        return aoFullName;
    }
    public String genJavaQuery(Class<?> domainClass, String actionContext, Writer out) {
        Map<String, Object> context = MapUtil.newHashMap();
        String domainUpper = StringUtil.uppercaseFirstLetter(DaoGenUtil.getDoAlias(domainClass));
        domainUpper = StringUtil.uppercaseFirstLetter(StringUtil.getLastBefore(domainUpper, DomainType.DTO));
        String aoClassName = domainUpper + "Query";
        ...
        Set<String> mappingFilters = new HashSet<String>();
        mappingFilters.add("class");
        List<IbatisResult> mappingColumns = getDbQUeryResult(domainClass, mappingFilters, OptionEnum.READ_AND_WRITE);
        String aoFullName = targetActionPackage + "." + aoClassName;
        String idType = genMetaInfo.getIdType();
        String idTypeUpper = StringUtil.uppercaseFirstLetter(idType);
        ...
        templateSourceGenator.genSource(context, out, genJavaQueryTemplateName);
        return aoFullName;
    }
    ...
    public void genJavaAction(String className, Class<?> domainClass, String actionContext, Writer out,
                              String actionContextPackage) {
        className = StringUtil.getLastBefore(className, DomainType.DTO);
        Map<String, Object> context = MapUtil.newHashMap();
        ...
        context.put("thisPackage", thisPackage);
        context.put("thisClassName", thisClassName + "Controller");
        context.put("voSimpleParam", StringUtil.lowercaseFirstLetter(genMetaInfo.getVoSimpleClassName()));
        context.put("querySimpleParam", StringUtil.lowercaseFirstLetter(genMetaInfo.getQuerySimpleClassName()));
        context.put("voSimpleClassName", genMetaInfo.getVoSimpleClassName());
        context.put("querySimpleClassName", genMetaInfo.getQuerySimpleClassName());
